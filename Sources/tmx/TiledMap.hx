...
class TiledMap {
	...
	public function new(data:String)
	{
	    ...
	    {
	    	// trace("that is String");
	    	source = new Fast(Xml.parse(data));
	    }
	    else if (Std.is(data, Xml))
	    {
	    	trace("that is xml dude");
	    	// source = new Fast(data);
	    }

	    source = source.node.map;

	    loadAttibutes(source);
	    loadProperties(source);
	    loadTilesets(source);
	    loadLayers(source);
	}

	public function loadAttibutes(source:Fast):Void
	{
		...
		bgColor = (source.has.backgroundcolor && source.att.backgroundcolor != null) ?
			Color.fromString(source.att.backgroundcolor) : null;
